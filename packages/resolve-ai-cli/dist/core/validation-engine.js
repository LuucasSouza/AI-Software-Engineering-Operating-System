...
const sensitivePatterns = [/^\.env($|\.)/i, /\.pem$/i, /\.key$/i, /\.p12$/i, /(^|\/)secrets(\/|$)/i, /(^|\/)backups(\/|$)/i, /_auth_users\.json$/i, /\.csv$/i, /\.xlsx$/i, /service-role/i, /credentials/i];
const skipDirs = new Set([".git", "node_modules", ".resolve-ai"]);
function normalizePath(filePath) { return filePath.replace(/\\/g, "/").replace(/^\.\//, ""); }
...
function walkSensitive(root, current = root, found = []) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory() && found.length < 50) walkSensitive(root, fullPath, found);
  }
...
function categorize(files) {
  const categories = { documentacao: [], codigo: [], configuracao: [], testes: [], sensivel: [], outros: [] };
  for (const file of files) categories[classify(file)].push(file);
  return categories;
}
function docsPresent(root) {
  const paths = resolveAiPaths(root);
  return ["15-tarefa-preparada.md", "20-execucao-assistida.md", "21-aprovacao-humana.md", "22-prompt-final-para-agente.md", "23-checklist-pos-execucao.md", "24-registro-de-execucao.md"].filter((file) => fs.existsSync(path.join(paths.docsDir, file))).map((file) => `docs/resolve-ai/${file}`);
}
...
export function buildValidationResult(root, state) {
  ...
  const riscosRestantes = [...previousRisks, ...(sensitiveFiles.length > 0 ? ["Arquivo sensível detectado por nome/caminho. Não commitar antes de revisão de segurança."] : []), ...(changedFiles.length > 0 ? ["Rodar testes e validações manualmente antes de commit."] : ["Não há evidência de mudança local para validar."])];
  const proximaAcao = status === "bloqueada" ? "Pare antes de commitar. Revise os arquivos sensíveis e remova ou proteja o que não deve ir para o repositório." : status === "pendente" ? "Execute a tarefa assistida ou revise o prompt antes de tentar validar novamente." : "Revise o checklist, rode os testes recomendados manualmente e confira o diff antes de commitar.";
  return {
    ...
    possibleOutOfScope,
    evidence: [gitAvailable ? "Git metadata disponível via git status --porcelain." : "Git metadata indisponível.", hasAssistedExecution ? "Execução assistida anterior encontrada no state." : "Nenhuma execução assistida anterior encontrada.", `${changedFiles.length} arquivo(s) alterado(s) detectado(s).`],
    notValidated: ["Testes não foram executados automaticamente nesta fase.", "Conteúdo de arquivos sensíveis não foi lido.", "Validação semântica profunda foi adiada para fase futura."],
    docsReference
  };
}
