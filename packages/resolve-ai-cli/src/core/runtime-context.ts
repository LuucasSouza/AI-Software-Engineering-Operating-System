...
import type { NextAction, ResolveAiState, RuntimeContext } from "../types/runtime.js";

function hasSensitiveRuntimeRisk(state: ResolveAiState): boolean {
...
function resolveProjectKind(root: string, state: ResolveAiState): string {
...
export function flowNameFor(projectKind: string, hasInterview: boolean): string {
...
function resolveProfile(state: ResolveAiState, projectKind: string): string {
  if (state.modoRecomendado) return state.modoRecomendado;
  if (projectKind === "novo") return "Non-Technical Builder";
  return "ainda não definido";
}

export function getPrimaryNextAction(context: RuntimeContext): NextAction {
  if (!context.isActive) {
    return {
...
export function gitTipFor(context: RuntimeContext): string | null {
...
export function buildRuntimeContext(root: string, state: ResolveAiState): RuntimeContext {
