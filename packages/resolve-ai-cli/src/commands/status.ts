...
import type { ResolveAiState } from "../types/runtime.js";

function projectLabel(root: string, state: ResolveAiState): string {
  if (state.ultimaEntrevista?.resumoCurto) return state.ultimaEntrevista.resumoCurto;
  return path.basename(root) || "este projeto";
}

function detailSections(root: string, state: ResolveAiState): string {
